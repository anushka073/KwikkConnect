...
const CaseFormModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Create case data for API
      const caseData = {
        title: formData.title,
        description: formData.description,
        priority: formData.severity,
        assignedTo: 'expert@example.com' // For demo, assign to our test expert
      };
      ...
      if (response.success) {
        ...
        try {
          if (formData.severity === 'critical' || formData.severity === 'high') {
            await notificationService.notifyHighPriorityCase(newCase);
          } else {
            await notificationService.notifyNewCase(newCase);
          }
        } catch (notificationError) {
          console.error('Failed to send notification:', notificationError);
        }

        // Save the new case to localStorage for demo
